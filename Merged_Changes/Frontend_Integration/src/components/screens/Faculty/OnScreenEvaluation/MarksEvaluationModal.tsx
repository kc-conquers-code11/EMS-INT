...
export interface MarksEvaluationModalProps {
  isOpen: boolean;
  onClose: () => void;
  seatNo: string;
  studentName: string;
...
export const MarksEvaluationModal: React.FC<MarksEvaluationModalProps> = ({ 
  isOpen, 
  onClose,
  seatNo,
  studentName,
  ...
  const [marksData, setMarksData] = useState<Record<string, { max: number, input: number | null }[]>>({
  ...
  const handleNumberClick = (valStr: string) => {
    ...
    const qData = marksData[activeInput.q];
  ...
  const totalMarks = Object.values(marksData).reduce((sum, q) => sum + q.reduce((acc, sub) => acc + (sub.input || 0), 0), 0);
  const maxTotal = Object.values(marksData).reduce((sum, q) => sum + q.reduce((acc, sub) => acc + sub.max, 0), 0);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-8">
      <div className="bg-white rounded-xl w-full h-full max-w-[1400px] flex flex-col p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-row gap-6 flex-1 min-h-0">
          ...
          <div className="w-[320px] flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
            ...
            <div className="grid grid-cols-2 gap-3 mb-2">
              <div className="bg-white border border-[#eaecf0] rounded-lg p-3 flex items-center justify-center text-center shadow-sm h-14">
                <span className="text-[13px] font-bold text-[#344054] leading-tight">Questions<br/>& Marks</span>
              </div>
              <div className="bg-white border border-[#eaecf0] rounded-lg p-3 flex items-center justify-center text-center shadow-sm h-14">
            ...
            {/* Questions Rendering */}
            {Object.keys(marksData).map((qKey) => (
              <div key={qKey} className="grid grid-cols-2 gap-3">
                <div className="bg-white border border-[#eaecf0] rounded-lg p-3 flex flex-col items-center justify-center shadow-sm min-h-[80px]">
                  <span className="text-[14px] font-bold text-[#101828] mb-1">{qKey}</span>
                  <div className="flex gap-2">
                    {marksData[qKey].map((sub, idx) => (
                  ...
                  <div className="flex gap-2 h-full items-center">
                    {marksData[qKey].map((sub, idx) => (
                      <button
                        ...
                        className={`text-[14px] font-bold px-2 py-1 rounded transition-colors min-w-[28px] min-h-[28px] flex items-center justify-center ${
            ...
            <button onClick={() => setIsLockConfirmOpen(true)} className="w-full py-3 bg-[#0E1680] text-white text-[14px] font-bold rounded-lg hover:bg-[#0E1680]/90 transition-colors shadow-sm mt-2">
              Lock Marks
            </button>
