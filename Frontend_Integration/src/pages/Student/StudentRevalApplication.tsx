...
export const StudentRevalApplication: React.FC = () => {
  ...
  const fetchEligibleSubjects = async () => {
    try {
      setIsLoading(true);
      const res = await axiosInstance.get('/student/revaluation/eligible-subjects');
      if (res.data?.success) {
  ...
  const handleApply = async (subject_mapping_id: string, component: string) => {
    const confirmApply = window.confirm(`Are you sure you want to apply for revaluation in ${component}? A standard fee applies.`);
    if (!confirmApply) return;

    setApplyingFor(`${subject_mapping_id}_${component}`);
    try {
      const res = await axiosInstance.post('/student/revaluation/apply', {
        subject_mapping_id,
        component
      });
      if (res.data?.success) {
      ...
      alert(err.response?.data?.message || 'Failed to apply for revaluation');
    } finally {
      setApplyingFor(null);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 animate-in fade-in duration-300 font-['Instrument_Sans']">
      <div className="flex flex-col gap-2 mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Revaluation Application</h1>
        <p className="text-sm text-gray-500">Apply for revaluation for subjects where your marks have been published.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-gray-500 flex flex-col items-center gap-3">
            ...
            <p>No eligible subjects found for revaluation at this time.</p>
          </div>
        ) : (
